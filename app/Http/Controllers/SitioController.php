...
class SitioController extends Controller
{
    ...
    public function consultaAgenda(Request $request){
        ...
        $timestamp          =   \Carbon\Carbon::parse($desde)->timestamp;
        $desde              =   date('Y-m-d', $timestamp);

        $hasta              =   str_replace('/', '-', $hasta);
        $timestamp          =   \Carbon\Carbon::parse($hasta)->timestamp;
        $hasta              =   date('Y-m-d', $timestamp);

        $desde              =   strtotime($desde);
        $hasta              =   strtotime($hasta);

        Session::put('agendadesde', $desde);
        Session::put('agendahasta', $hasta);
    ...
    public function crearSemana($medico_id,$fechadesde,$fechahasta,$empresa)
    {
        ...
        // dd(date('Y-m-d',Session::get('agendahasta')));
        ...
        while(strtotime($empresa->fin_atencion) > strtotime($inicio_atencion))
        {
            ...
            for($i=0;$i<7;$i++){
                    ...
                    // echo date('Y-m-d',Session::get('agendahasta'))."::".($fecha_dia)."<br>";
                    if(Session::get('agendahasta') < strtotime($fecha_dia) or Session::get('agendadesde') > strtotime($fecha_dia) ){
                        $semana         .= "<td style='background-color:#d1cece' > <span class=\"label label-primary\">&nbsp;</span> </td>";
                    }elseif(!$this->getHorarioLaboralxId($medico_id,($i),1,$inicio_atencion_ant)){// TODO CONSULTAR LOS HORARIOS LABORABLES
                        $semana         .= "<td style='background-color:#d1cece' > <span class=\"label label-primary\">&nbsp;</span> </td>";
